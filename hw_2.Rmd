...
nnet <- train(y ~ ., method='nnet', data = training_set, metric='Accuracy',preProcess = preProcess, trControl=trControl)
glm <- train(y ~ ., method='glm', data = training_set, metric='Accuracy',preProcess = preProcess, trControl=trControl)
```
...
accuracy_knn <- posPredValue(test_set$y, test_set$pred_knn)
accuracy_rf <- posPredValue(test_set$y, test_set$pred_rf)
accuracy_nnet <- posPredValue(test_set$y, test_set$pred_nnet)
accuracy_glm <- posPredValue(test_set$y, test_set$pred_glm)
```
...
| knn | `r accuracy_knn ` |
| rf | `r accuracy_rf ` |
| nnet | `r accuracy_nnet ` |
| glm | `r accuracy_glm ` |

We can see that the best performance is yielded by the ... model.
...
```{r}
test_set$pred_ensemble <- as.factor(ifelse(test_set$pred_nnet=="suburb", ifelse(test_set$pred_glm=="suburb", "suburb", as.character(test_set$pred_glm)), ifelse(test_set$pred_glm=="suburb", as.character(test_set$pred_glm), "city")))

accuracy_ensemble <- posPredValue(test_set$y, test_set$pred_ensemble)
```
...
Let us see now the confusion matrix.
